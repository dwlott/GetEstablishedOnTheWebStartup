...
2. **Robocopy** — owner runs locally with purpose-appropriate flags:

| Purpose | Robocopy |
| --- | --- |
| review, portability | `/MIR` (purge extras) |
| backup | `/E` (no purge) |

```powershell
$Source = "<local-repo-root>"
$Dest   = "<mirror-target-path>"
$Purpose = "backup"   # or review | portability

$XD = @(".git", "node_modules", ".venv", "Intake", "IncomingIdeas")
$XF = @(".env", "*.pem", "*.key")

if ($Purpose -eq "backup") {
