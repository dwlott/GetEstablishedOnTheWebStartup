...
When copying GEOTW → startup, exclude these paths at robocopy time to avoid
copy-then-delete churn:
...
$xf = @()
robocopy $src $dst /E /XD $xd /XF $xf
```

After copy, still run the remove matrix for any paths that slipped through.
...
| `Plans/AgentTaskBacklog.md` | Starter Ready Next; no host migration queue |
| `Plans/OpenQuestions.md` | Consumer banner at top; reset launch decisions |
