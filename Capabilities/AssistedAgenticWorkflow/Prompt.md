...
Repository: GetEstablishedOnTheWebStartup
Branch: main (if .git exists)
Local write path: {{YOUR_REPOSITORY_ROOT}}
Cloud inspection path: {{YOUR_CLOUD_MIRROR_PATH}}
  (read-only connector inspection only — not the write target unless owner
  explicitly approves cloud mutation)

Read first:
- AGENTS.md
- Capabilities/SituationalAwareness/Rules.md
- Capabilities/AssistedAgenticWorkflow/Rules.md
- Plans/AgentTaskBacklog.md (or the named task file)
...
- Do not modify capability-provisioned Inbox folders unless Setup ran and owner
  approved that scope.
...
- Start with max_depth=1 listing of the owner-named cloud mirror root.
