...
- Page intro typography (via `page-layout-manifest.php` `intro_paragraphs`):
  paragraph 1 → `geotw-page-lead` (large headline); paragraph 2 →
  `geotw-page-deck` (italic side-accent); paragraph 3 → `geotw-page-pitch`
  (larger callout box — use for benefit/free emphasis; bold key words with
  `**`).
